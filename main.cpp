...
Logger::Logger(const char* filename) : m_filename(filename)
{
    m_sm.lock();
    m_filename += ".txt";
    m_file.open(m_filename, std::ios_base::in | std::ios_base::out | std::ios_base::trunc);     //пока сделал так, чтобы каждый раз очищался
    if (m_file.is_open())
    {
...
Logger::~Logger()   //в такой реазилации не нужен, т.к. close делается после каждого чтения или записи
{
    m_sm.lock();
...
{
    std::this_thread::sleep_for(std::chrono::milliseconds(100));    //в учебных целях
    m_sm.lock();
    m_file.open(m_filename, std::ios_base::in | std::ios_base::out);
    if (m_file.is_open())
    {
        //
        m_file.seekp(0, std::ios_base::end); //write to te end of file
        m_file << msg << "\n";
        //
    }
    else
...
{
    std::this_thread::sleep_for(std::chrono::milliseconds(100));   //в учебных целях
    //m_sm.lock_shared();     //изначально писалось на с++11
    m_sm.lock();            //ЗАМЕНИТЬ НА shared_lock() в c++17;
    m_file.open(m_filename, std::ios_base::in | std::ios_base::out);
    if (m_file.is_open())
    {
        //
        m_file.seekg(0, std::ios_base::beg); //reading from the beginning
        std::string tmp;
    ...
    m_file.close();
    //m_sm.unlock_shared();
    m_sm.unlock();              //ЗАМЕНИТЬ НА shared_unlock() в c++17;
}
...
int main()
{
    ...
    //Ждём завершения записи всех данных и считываем всё
